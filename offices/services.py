...
# 42개 시/구청 민원실 실시간 대기 분류 기준 통합 매핑
API_TASK_MAPPING = {
    '🛂 여권 발급 및 교부':      ['여권'],
    '📄 증명서 발급 및 통합민원': ['증명', '등초본', '등 초본', '인감', '통합민원', '민원발급'],
    '💍 가족관계 (혼인/출생 등)': ['가족', '가족관계', '호적', '혼인', '출생', '사망', '이혼'],
    '🚗 자동차 등록 및 세무':    ['차량', '자동차', '취득세', '등록세', '세무'],
}
...
def recommend_offices_general(
    ...
    # 대한민국 행정 기준: 혼인/이혼은 구청(시청), 장애인/기초수급 등 복지는 행정복지센터(동주민센터)
    CITY_HALL_SERVICES = ['여권 발급·재발급', '부동산 거래 신고', '자동차 취득세 신고', '혼인 신고', '이혼 신고']
